...
active_rbd_t active_rbds[MAX_IMAGES];
...
static void bs_rbd_request(struct scsi_cmd *cmd)
{
	...
	ret = length = 0;
	key = asc = 0;
	active_rbd_t *rbd = RBDP(cmd->dev->fd);
...
/* XXX defined in bs_rdwr.c; maybe in bs.c? */
...
static tgtadm_err bs_rbd_init(struct scsi_lu *lu)
{
	...
	if ((rados_ret = rados_create(&cluster, NULL)) < 0) {
		eprintf("bs_rbd_init: rados_create: %d\n", rados_ret);
		return ret;
	}
	/* read config from environment and then default files */
	if ((rados_ret = rados_conf_parse_env(cluster, NULL)) < 0) {
		eprintf("bs_rbd_init: rados_conf_parse_env: %d\n", rados_ret);
		goto fail;
	}
	if ((rados_ret = rados_conf_read_file(cluster, NULL)) < 0) {
		eprintf("bs_rbd_init: rados_conf_read_file: %d\n", rados_ret);
		goto fail;
	}
	if ((rados_ret = rados_connect(cluster)) < 0) {
...
__attribute__((constructor)) static void bs_rbd_constructor(void)
{
